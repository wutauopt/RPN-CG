...
function [xopt, fs, Ds, iter, sparsity, comtime, iter_time] = driver_rpn_cgh(H, option,d_l, V)
    % parameters
    n = option.n;
    r = option.r;
    mu = option.mu;
    ...
    t = 1/L;
    tic
    [xopt, fs, Ds, iter, iter_time] = solver(H, fhandle, gfhandle, fcalA, fcalAstar, fprox, fcalJ, Bv, x, t, tau, tol, maxiter, mu, stop_label, epsilon, params, option);
    comtime = toc;
    xopt.main(abs(xopt.main) < 1e-5) = 0;
    sparsity = sum(sum(abs(xopt.main) < 1e-5)) / (n * r);
    xopt = xopt.main;
    fprintf('RPN-CGH:*** Iter ***  Fval *** CPU  **** sparsity *** opt_norm  \n');
    print_format = '     %i     %1.5e    %1.2f        %1.2f        %1.3e       \n';
    fprintf(1,print_format, iter,fs(end), comtime, sparsity, Ds(end));
end

function [xopt, fs, Ds, iter, iter_time] = solver(H, fhandle, gfhandle, fcalA, fcalAstar, fprox, fcalJ, Bv, x0, t, tau, tol, maxiter, mu, stop_label, epsilon, params, option)
    err = inf;
    x1 = x0;
    x2 = x1; fs = []; Ds = [];
    [f1, x1] = fhandle(x1);
    ...
    while(err > stop_label &&  iter < maxiter)
        ...
        if(nv > epsilon) % use ManPG-ada
            ...
            [f2, x2] = fhandle(x2);
            btiter = 0;
            while(f2 > f1 - params.rho1 * alpha * nvsquared && btiter < 3)
                linesearch_flag = 1;
                alpha = alpha * params.rho2;
                x2 = R(x1, alpha * D);
                [f2, x2] = fhandle(x2);
                btiter = btiter + 1;
                totalbt = totalbt + 1;
