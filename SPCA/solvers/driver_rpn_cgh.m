...
function [xopt, fs, Ds, iter, sparsity, comtime, iter_time] = driver_rpn_cgh(A, option)
    % parameters
    n = option.n;
    r = option.r;
    mu = option.mu;
    ...
    t = 1/L; 
    tic
    [xopt, fs, Ds, iter, iter_time] = solver(A, fhandle, gfhandle, fcalA, fcalAstar, fprox, fcalJ, Bv, x, t, tau, tol, maxiter, mu, stop_label, epsilon, params, option);
    comtime = toc;
...
function [xopt, fs, Ds, iter, iter_time] = solver(A, fhandle, gfhandle, fcalA, fcalAstar, fprox, fcalJ, Bv, x0, t, tau, tol, maxiter, mu, stop_label, epsilon, params, option)
    err = inf;
    x1 = x0;
    x2 = x1; fs = []; Ds = [];
    [f1, x1] = fhandle(x1);
